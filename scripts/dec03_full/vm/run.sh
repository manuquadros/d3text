...
configure () {
  ...
  fi
  {
    ...
    echo ""
    echo "# 0, not the 6300 documents (~91 GB) the earlier baseline used:"
    echo "# with the store serving every document there is nothing for the RAM"
    echo "# cache to hold that the store is not already holding, and 80 GiB of"
    echo "# resident set buys nothing."
