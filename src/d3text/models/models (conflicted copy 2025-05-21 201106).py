    ...
    def train_model(
        ...
        for epoch in trange(
            ...
            for batch in tqdm(
                ...
                n_batches += 1
                # tqdm.write(f"Training loss: {loss.item():.2e}")
                ...
                #         print(
                #             f"{name}: grad norm = {param.grad.norm().item():.3f}"
                #         )
...
class ETEBrendaModel(Model):
    ...
    def ground_truth(
        ...
        # # assign class target for non-entities
        # mask = (class_targets == 0).all(dim=-1)
        # indices = mask.nonzero(as_tuple=True)[0]
        # class_targets[indices, ..., -1] = 1
    ...
    def forward(self, input_data: dict) -> tuple[Tensor, Tensor]:
        ...
        with torch.autocast(device_type=self.device):
            ...
            # tqdm.write(f"Hidden mean: {x.mean().item()}, std: {x.std().item()}")
            ...
            # tqdm.write(
            #     f"Classifier weight norm: {self.entity_classifier.weight.norm().item()}"
            # )
    ...
    def evaluate_model(
        ...
        with torch.no_grad():
            for batch in tqdm(test_data):
                ...
                class_preds.append(
                    ...
                    .scatter(1, class_pred.unsqueeze(1), 1)
                    .cpu()
                    .numpy()
                )
                class_gts.append(gt_classes.squeeze().cpu().numpy())

        ent_preds = numpy.vstack(ent_preds).astype(int)
        ent_gts = numpy.vstack(ent_gts).astype(int)
        # print("First batch preds sum:", ent_preds.sum())
        # print("First batch gts sum:", ent_gts.sum())
        ...
        # print("First batch class preds sum:", class_preds.sum())
        # print("First batch class gts sum:", class_gts.sum())
        print(
            classification_report(
                class_gts,
                class_preds,
