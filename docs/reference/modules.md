...
can be imported without the BRENDA data layer coming along. `d3text.constraints` and `d3text.schema`
...
| `d3text.datasets.{s800,enzymener,expasy,nlp4pheno,culture_numbers}` | External corpus loaders | `d3text.identifier_bridge`; `culture_numbers` also `d3text.surface_forms` |
| `d3text.data.data` | `BrendaDataset`, `TokenBudgetBatchSampler`, `collate_documents` | `d3text.encodings_store`, `d3text.utils`, `d3text.models.model_types` |
...
| `d3text.linking_eval` | Scoring the linker against an external corpus | `d3text.identifier_bridge`, `d3text.linking`, `d3text.mention_metrics`, `d3text.token_labels` |
| `d3text.annotation_hub` | Predictions mapped onto annotation-hub's `POST /save/` object | `d3text.corpus`, `d3text.identifier_bridge` |
| `d3text.linking_corpora` | Finding and scoring every configured corpus | `d3text.linking_eval`, `d3text.linking`, `d3text.identifier_bridge`, `d3text.datasets`, `d3text.corpus`, `d3text.surface_forms`; `brenda_references` for its data directory |
| `d3text.taxonomy` | `merged_taxids` — forwarding retired NCBI taxids | leaf (ncbitax) |
...
| `d3text.utils` | Window aggregation, tokenizer loading | `d3text.runtime`; torch, transformers |
...
| `lpsn_interface` | git dependency (transitive) | LPSN taxonomy adapter |
